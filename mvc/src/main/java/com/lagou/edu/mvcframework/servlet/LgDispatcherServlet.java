...
public class LgDispatcherServlet extends HttpServlet {
    ...
    //全局黑名单
    private List<String> handlerMappingBlack = new ArrayList<>();
    //全局白名单
    private List<String> handlerMappingWhiteGloab = new ArrayList<>();
    ...
    public void init(ServletConfig config) throws ServletException {
        ...
        //初始化黑名单
        handlerMappingBlack.add(properties.getProperty("blacklist"));
        // 2 扫描相关的类，扫描注解
    ...
    private void initHandlerMapping() {
        ...
        for(Map.Entry<String,Object> entry: ioc.entrySet()) {
            ...
            for (int i = 0; i < methods.length; i++) {
                ...
                //  方法如果有标识MySecurity，就对比黑白名单做限制访问处理
                if(method.isAnnotationPresent(MySecurity.class)){
                    List<String> handlerMappingWhite = new ArrayList<>();
                    //设置该方法有MySecurity注解，需要后续做验证权限逻辑处理
                    handler.setHasMySecurity(true);
                    MySecurity securityAnnotation = method.getAnnotation(MySecurity.class);
                    String[] value = securityAnnotation.value();
                    for (String s : value) {
                        handlerMappingWhite.add(s);
                    }
                    //封装可以访问接口的白名单用户
                    handler.setHandlerMappingWhite(handlerMappingWhite);
                }
            ...
            for (int i = 0; i < classNames.size(); i++) {
                ...
                if(aClass.isAnnotationPresent(MySecurity.class)){
                    ...
                    if(whiteList == null){
                        ...
                        handlerMappingWhiteGloab = Arrays.asList(whiteList);
                    }
                }
...
//        method.invoke() //
        ...
        //如果越权需展示页面提示
        if(handler.getAutoItecepter()){
            resp.getWriter().write("I'm so sorry I love you, but you don't have authority");
            return;
        }
    ...
    private Handler getHandler(HttpServletRequest req) {
        ...
        for(Handler handler: handlerMapping) {
            ...
            //只有MySecurity注解的函数才会走验证逻辑
            if(handler.getHasMySecurity()){
                //权限验证逻辑
                if(handlerMappingBlack.contains(username)){
                    handler.setAutoItecepter(true);
                } else if (handler.getHandlerMappingWhite().contains(username)||handlerMappingWhiteGloab.contains(username)){
                    handler.setAutoItecepter(false);
                }else {
                    handler.setAutoItecepter(true);
                }
            }
